...
public class AlunoDAO {
	...
	public ArrayList<AlunoTO> listarAlunos() {
		...
		String sqlSelect = "SELECT codigo, nome, dataNascimento, endereco, numero, complemento, rg, cpf, telefone, email FROM aluno";
	...
	public ArrayList<AlunoTO> listarAlunos(String chave) {
		AlunoTO to;
		ArrayList<AlunoTO> lista = new ArrayList<>();
		String sqlSelect = "SELECT codigo, nome, dataNascimento, endereco, numero, complemento, rg, cpf, telefone, email FROM aluno where upper(nome) like ?";
		// usando o try with resources do Java 7, que fecha o que abriu
		try (Connection conn = ConnectionFactory.obtemConexao();
				PreparedStatement stm = conn.prepareStatement(sqlSelect);) {
