...
public ArrayList<CursoTO> listarCursos(String chave) {
	CursoTO to;
	ArrayList<CursoTO> lista = new ArrayList<>();
	String sqlSelect = "SELECT codigo, nome, tipo, dataDeInicio, dataDeTermino, horario, numeroDeVagas, valor, tipoLab, softwares, livros, descricaoMaterial FROM curso where upper(nome) like ?";
	// usando o try with resources do Java 7, que fecha o que abriu
	try (Connection conn = ConnectionFactory.obtemConexao();
			PreparedStatement stm = conn.prepareStatement(sqlSelect);) {
